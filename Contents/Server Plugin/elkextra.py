		...
		if zdAddr in existaddrs:
			for device in existdevs:
				if zdAddr == device.address:
					indigo.server.log("Updating existing device %s, %s" % (device.name, zone[1]))
					device.replacePluginPropsOnServer({"address":zdAddr, "type":zone[1]})
					if autoNames:
			...
			try:
				indigo.device.create(protocol=indigo.kProtocol.Plugin,
					...
					folder=folderId)
				indigo.server.log("Creating device - Name: %s, Zone #: %s" % (zdName, zdAddr))
			except ValueError:
