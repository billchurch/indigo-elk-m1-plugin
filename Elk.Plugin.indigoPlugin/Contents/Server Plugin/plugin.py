...
#					self.errorLog("Unknown Error: %s" % sys.exc_info()[0])
	...
	def elkstartup(self):
		try:
			...
			self.folderId = elkextra.createFolder(self.pluginPrefs["deviceFolder"])
			self.debugLog("Getting 'Alarm Panel' device id.")
			self.panelId = elkextra.createPanel(self.pluginPrefs["autoPanel"], self.pluginPrefs["ip_address"], self.pluginPrefs["ip_port"], self.folderId)
			self.debugLog("Folder id: %s, Alarm Panel id: %s" % (self.folderId, self.panelId))
		except KeyError:
		...
		if self.panelId > 0:
			...
			try:
				...
				self.debugLog("Connected to panel, sending arming state request.")
				self.ePanel.sendCmd("06as00")
				self.debugLog("Processing arming state request.")
				self.dispatchMsg(self.ePanel.readData())
			except socket.error, e:
				self.errorLog("Unable to connect to alarm panel. %s" % e.message)
				indigo.devices[self.panelId].updateStateOnServer("conn_state", "Off")
		...
		if (self.pluginPrefs["thermostats"] or reqThermoData) and self.pluginPrefs["startupValidate"]:
			...
			self.debugLog("Startup validation disabled, not checking thermostats.")
		if self.pluginPrefs["subscribe"]:
			self.debugLog("Startup validation enabled, starting subscription.")
			indigo.devices.subscribeToChanges()
		else:
			self.debugLog("Startup validation disabled, subscription disabled.")
...
# 				print "Enter Exit state is %s" % eeState
				...
				self.HoldFeedback = False
 				...
 				elkextra.setDeviceDesc(daddr, id, name, self.pluginPrefs["autoNames"])
			elif 'ST' in msg:
				...
				pass
 				...
 				self.debugLog(msg.rstrip())
 				tnum, mode, hold, tfan, temp, heat, cool = self.ePanel.thermoRpt(msg)
 				if int(temp) != 0:
 					elkextra.setThermoInfo(tnum, mode, hold, tfan, temp, heat, cool, self.folderId)
#			elif 'VN' in msg:
...
#				indigo.server.log(msg.rstrip())
				...
				self.debugLog(str(zoneD))
 				elkextra.setupZones(zoneD, self.folderId, self.pluginPrefs["autoNames"])
 			elif 'ZS' in msg:
 				self.debugLog(msg.rstrip())
	...
	def readDeviceDesc(self, device):
		if device.address != "":
			self.debugLog("Processing string data request for %s." % device.name)
			cmdPrefix = "0Bsd"
			...
			self.errorLog("Attempt was made to request name of undefined device %s." % device.name)
		...
		if typeId == 'alarmPanel':
			...
			if ipOK and portOK:
				if self.panelId > 0:
					...
					device.replacePluginPropsOnServer({"address":newaddr,"ip_address":valuesDict["ip_address"],"ip_port":valuesDict["ip_port"]})
				rtn = True
	...
	def deviceUpdated(self, origDev, newDev):
		...
		if (origDev.model=="Universal Module Receiver"):
			# Received an On command for a device labeled as a Universal Module Receiver
			...
			# Received an Off command for a device labeled as a Universal Module Receiver
